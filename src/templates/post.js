...
const buildOptions = currentURL => {
  const options = {
    ...
    renderNode: {
      [BLOCKS.HEADING_1]: (node, children) => <h1 className='meu-h1'>{children}</h1>,
      [BLOCKS.PARAGRAPH]: (node, children) => <p className='meu-p'>{children}</p>,
...
const Post = ({ data, path }) => {
  return (
    <div>
      ...
      { /*<img src={data.contentfulPost.featureImage.fixed.src} /> */}
      <Image fixed={data.contentfulPost.featureImage.fixed} />
